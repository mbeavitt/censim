...
def _extract_and_sort_windows_numba_parallel(padded_D, window_size, n_windows):
    """
    Extract upper triangle distances from all windows and sort them in parallel.
...
@njit(parallel=True, fastmath=True)
def _compute_correlation_sums_batch_parallel(sorted_distances, r_values, window_size):
    """
    Compute correlation sums for all windows using pre-sorted distances (parallel).
...
def sliding_window_local_correlation(D, window_size, r_values, use_parallel=False):
    """
    ...
    correlation sums. Can use serial or parallel mode.
    ...
    # Choose functions based on parallel mode
    if use_parallel:
        extract_func = _extract_and_sort_windows_numba_parallel
        compute_func = _compute_correlation_sums_batch_parallel
    else:
        extract_func = _extract_and_sort_windows_numba_serial
        compute_func = _compute_correlation_sums_batch_serial

    # Extract and sort all windows (numba optimized)
    sorted_distances = extract_func(padded_D, window_size, n_windows)

    # Compute correlation sums for all windows (numba optimized)
    mean_correlations = compute_func(sorted_distances, r_values, window_size)
